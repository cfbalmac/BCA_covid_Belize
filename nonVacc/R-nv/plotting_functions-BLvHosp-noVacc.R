...
run_fits <- function(rep_plot,nn,cut_off,dt,filename="1_nvacc"){
  ...
  S_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  I_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  H_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  R_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  C_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  Rep_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  R0_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  Rec_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  CC_plot_nv = matrix(NA,ncol=rep_plot,nrow=t_period)
  ...
  save(
    ...
    Rec_plot_nv,
    CC_plot_nv,
    file=paste0("nonVacc/outputs-nv/bootstrap_fit_",filename,".RData")) 
...
plot_outputs <- function(filename="1_nvacc"){
  ...
  H_quantile_nv <- apply(H_plot_nv,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975),na.rm = TRUE)})
  ...
  R0_quantile_nv <- apply(R0_plot_nv,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975),na.rm = TRUE)}) #*S_plot/theta[["pop"]]
  R0_average_nv <- apply(R0_plot_nv,1,mean)
  
  Case_quantile_nv <- apply(C_plot_nv,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975),na.rm = TRUE)}) 
  Case_quantile_onset_nv <- theta[["onset_prop"]]*Case_quantile_nv
  Case_average_onset_nv <- theta[["onset_prop"]]*apply(C_plot_nv,1,mean)
  
  CCase_quantile_nv <- apply(CC_plot_nv,1,function(x){quantile(x,c(0.025,0.25,0.5,0.75,0.975),na.rm = TRUE)}) 
  CCase_quantile_onset_nv <- theta[["onset_prop"]]*CCase_quantile_nv
  CCase_average_onset_nv <- theta[["onset_prop"]]*apply(CC_plot_nv/10,1,mean)
  
  
  # Remove final few points (as estimation less reliable)
  S_quantileA <- S_quantile_nv[,1:(ncol(R0_quantile_nv)-cut_off)]
  Inf_quantileA <- Inf_quantile_nv[,1:(ncol(R0_quantile_nv)-cut_off)]
  H_quantileA <- H_quantile_nv[,1:(ncol(R0_quantile_nv)-cut_off)]
  R0_quantileA <- R0_quantile_nv[,1:(ncol(R0_quantile_nv)-cut_off)]
  date_rangeA <- date_range[1:(length(date_range)-cut_off)]
  Case_quantile_onsetA <- Case_quantile_onset_nv[,1:(length(date_range)-cut_off)]
  CCase_quantile <- CCase_quantile_nv[,1:(length(date_range)-cut_off)]
  
  save(
    R0_average_nv,
    R0_quantile_nv,
    Case_quantile_nv,
    Case_quantile_onset_nv,
    Case_average_onset_nv,
    CCase_quantile_nv,
    CCase_quantile_onset_nv,
    CCase_average_onset_nv,
    S_plot_nv,
    I_plot_nv,
    H_plot_nv,
    R_plot_nv,
    ...
    R_quantile_cum_nv,
    file=paste0("nonVacc/outputs-nv/results_",filename,".RData")) 
  ...
  plot(date_rangeB,R0_quantileB[1,],col="white",ylim=c(0,10),xlim=c(xMin1,xMaxR),xlab="",ylab=expression(paste(R[t])))
  ...
  dev.copy(png,paste("nonVacc/plots-nv/reproduction_number.png",sep=""),units="cm",width=10,height=15,res=150)
  dev.off()
  ...
  lines(date_rangeA,Case_average_onset_nv,type="l",col="darkgreen",xaxt="n",yaxt="n",xlab="",ylab="",lty=1,lwd=3)
  lines(date_rangeA,H_quantile_n_average_nv,type="l",col="blue",xaxt="n",yaxt="n",xlab="",ylab="",lty=1,lwd=3)
  ...
  dev.copy(png,paste("nonVacc/plots-nv/new_cases.png",sep=""),units="cm",width=10,height=15,res=150)
  dev.off()
  ...
  dev.copy(png,paste("nonVacc/plots-nv/susceptibles.png",sep=""),units="cm",width=10,height=15,res=150)
  dev.off()
  
  
  # Plot case total predictions
  ym1 <- 50000
  plot(date_range,CCase_quantile_onset_nv[1,],col="white",ylim=c(0,ym1),xlim=c(xMin1,xMax),xlab="",ylab="cases in Belize")
  
  polygon(c(date_rangeF,rev(date_rangeF)),c(yyF,rev(yyF+1e5)),lty=0,col=rgb(0.9,0.9,0.9))
  
  polygon(c(date_range,rev(date_range)),c(CCase_quantile_onset_nv[2,],rev(CCase_quantile_onset_nv[4,])),lty=0,col=rgb(0,0.3,1,0.35))
  polygon(c(date_range,rev(date_range)),c(CCase_quantile_onset_nv[1,],rev(CCase_quantile_onset_nv[5,])),lty=0,col=rgb(0,0.3,1,0.2))
  lines(date_range,CCase_quantile_onset_nv[3,],type="l",col=rgb(0,0,1),xaxt="n",yaxt="n",xlab="",ylab="")
  ...
  dev.copy(png,paste("nonVacc/plots-nv/total_infections.png",sep=""),units="cm",width=10,height=15,res=150)
  dev.off()
  
  
  # output figure
  #dev.copy(png,paste("nonVacc/plots-nv/cases_inference.png",sep=""),units="cm",width=18,height=18,res=150)
  #dev.copy(pdf,paste("nonVacc/plots-nv/Figure_2.pdf",sep=""),width=7.5,height=8)
  #dev.off()
...
r0_value_output <- function(filename="1"){
  ...
  write_csv(out_r,"nonVacc/outputs-nv/before_after_R.csv")
...
plot_distn <- function(){
  ...
  dev.copy(pdf,paste("nonVacc/plots-nv/Figure_S1.pdf",sep=""),width=8,height=3)
  dev.off()
