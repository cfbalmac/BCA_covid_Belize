...
process_model <- function(t_start,t_end,dt,theta,simTab,simzetaA,vaccF,recover_hosp){
  ...
  # simTab <- storeL[,tt-1,]; t_start = 1; t_end = 2; dt = 0.1; simzetaA <- simzeta[1,]; vaccF=theta[["vacc_frac"]]; recover_hosp=recover_hosp
  ...
  for(ii in seq((t_start+dt),t_end,dt) ){
    ...
    exposed_t1 <- exposed_t1 + S_to_E1*(vacc_frac)*theta[["OR.vacc"]] - E1_to_E2
...
smc_model <- function(theta,nn,dt=1,seed=NULL){
  ...
  for(tt in 2:ttotal){
    ...
    # vaccination?
    if(tt<vacc_time){vaccF <- theta[["vacc_frac"]]}else{vaccF <- 1}
    
    if(tt>vacc_time){theta[["OR.vacc"]] <- theta[["OR.vacc"]]}else{theta[["OR.vacc"]] <- 1}
    #if(tt>vacc_time){theta[["recover"]] <- 1/8 }else{theta[["recover"]] <- theta[["recover"]]}
    
    if(tt<vacc_time){recover_hosp <- 14.26}else{recover_hosp <- 14.26}
    ...
    nnn <- length(W[1:nn,tt])
    ru <- (runif(1) + (0:(nnn-1))) / nnn
    cum_weights <- cumsum(W[1:nn,tt])
    
    A[, tt] <- findInterval(ru,cum_weights)+1
...
AssignWeights <- function(data_list,storeL,nn,theta,tt){
  ...
  local_case_data_tt_scale <- 1.5#data_list$local_case_data_onset_scale[tt] # deprecated
